...
export function persistGame() {
  const { checkpoint, camera, quality, reduced, muted, discovered, tapControls, desktopMode } = useGame.getState();
  try { localStorage.setItem(STORAGE, JSON.stringify({ checkpoint, camera, quality, reduced, muted, discovered, tapControls, desktopMode })); }
