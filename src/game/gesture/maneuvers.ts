...
import {
  ...
  TURN_DODGE, TURN_DUR_BASE, TURN_DUR_K, TURN_MAX_DEG, TURN_MAX_PX, TURN_MIN_DEG, TURN_MIN_PX,
} from './tuning';

export type ManeuverId = 'none' | 'turn' | 'soar' | 'dive' | 'pullout' | 'roll' | 'nudge';
...
/** Swipe length -> analog tier: linear from TURN_MIN_PX to TURN_MAX_PX (spec 6: 60-300 px). */
export const swipeMag = (px: number) => clamp01((px - TURN_MIN_PX) / (TURN_MAX_PX - TURN_MIN_PX));
...
export interface Maneuver {
  id: ManeuverId;
  /** Seconds into the program, and its length (Soar includes SOAR_SETTLE_S). */
  t: number; dur: number;
  /** Turn: +1 left (positive yaw, as runtime.yaw). Roll: +1 right (clockwise stroke). */
  sign: number;
  /** Turn angle, rad; Soar/Dive climb length, s. */
  angle: number;
  /** Peak offset speed, m/s. */
  peak: number;
  /** Roll: the sidestep was refused by pathClear, so it is a pure body roll. */
  blocked: boolean;
  /** Roll: waiting for the scheme's pathClear check on the next step. */
  pending: boolean;
  /** Nudge direction: camera right and world up components (unit). */
  rx: number; uy: number;
  mag: number;
}
...
export const createManeuver = (): Maneuver =>
  ({ id: 'none', t: 0, dur: 0, sign: 1, angle: 0, peak: 0, blocked: false, pending: false, rx: 0, uy: 0, mag: 0 });
...
function begin(m: Maneuver, id: ManeuverId, dur: number, sign: number, mag: number): Maneuver {
  ...
  m.angle = 0; m.peak = 0; m.blocked = false; m.pending = false; m.rx = 0; m.uy = 0;
  return m;
}
...
export function stepManeuver(m: Maneuver, dt: number, groundBelow: number, o: ManeuverOut): ManeuverOut {
  ...
  switch (m.id) {
    case 'turn':
      o.yawRate = m.sign * (m.angle / m.dur) * 6 * clamp01(u) * (1 - clamp01(u));
      o.right = -m.sign * m.peak * bump(u);
      break;
    case 'soar':
    ...
    case 'roll':
      ...
      break;
    case 'nudge': {
      const s = m.peak * bump(u);
      o.right = m.rx * s; o.up = m.uy * s;
      break;
    }
  }
  m.t += dt;
