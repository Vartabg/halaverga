...
export default function TrackpadSettings() {
  ...
  return <fieldset><legend>Trackpad controls</legend>
    <label className={styles.setting}>Trackpad steering<select value={state.trackpadSteering} onChange={e => save({ trackpadSteering: e.target.value as TrackpadProfile })}>
      <option value="simple">One finger + keyboard · recommended</option><option value="free">Classic · Free cursor</option><option value="captured">Classic · Captured steering</option><option value="flow">Flow · scroll experiment</option>
    </select></label>
    <p className={styles.muted}>{state.trackpadSteering === 'simple' ? 'Click the scene to lift into hover, then slide one finger to look. Hold WASD to move; release to hover. R/F rise and descend, Shift toggles Surge, Space lands. Click again to stop and free the pointer; Escape pauses. Scrolling does not control flight.' : state.trackpadSteering === 'flow' ? 'Slide to look, stroke to glide, press to hover. Looking stays free when you stop. Two-finger click releases the pointer for buttons; Escape pauses.' : state.trackpadSteering === 'free' ? `Click to cruise, move to steer, and scroll for speed. Click again to hover. ${state.sustainedEdges ? 'Turning continues while the pointer stays at an edge.' : 'Edge turns fade when pointer movement stops.'}` : 'Click to cruise with unlimited turning. The pointer hides during flight. Click again to brake and release it; then use any button normally. Escape pauses.'}</p>
