...
export default function TrackpadSettings() {
  ...
  return <fieldset><legend>Trackpad comparison</legend>
    <label className={styles.setting}>Trackpad steering<select value={state.trackpadSteering} onChange={e => save({ trackpadSteering: e.target.value as 'free' | 'captured' })}>
      <option value="free">A · Free cursor</option><option value="captured">B · Captured steering</option>
    </select></label>
    <p className={styles.muted}>{state.trackpadSteering === 'free' ? `Click to cruise, move to steer, and scroll for speed. Click again to hover. ${state.sustainedEdges ? 'Turning continues while the pointer stays at an edge.' : 'Edge turns fade when pointer movement stops.'}` : 'Click to cruise with unlimited turning. The pointer hides during flight. Click again to brake and release it; then use any button normally. Escape pauses.'}</p>
    ...
    <label className={styles.setting}>Starting cruise speed (m/s)<input type="number" min="3" max="34" step="1" value={state.cruiseSpeed} onChange={e => {
      const value = e.target.valueAsNumber; if (Number.isFinite(value)) save({ cruiseSpeed: Math.max(3, Math.min(34, value)) });
    }} /></label>
    <p className={styles.muted}>Use the same starting speed and arrival terrace for both tests. Brisk scroll strokes add bounded speed; pause and zoom always clear flight input.</p>
