...
import { notePointer, touchMode } from '@/game/pointerMode';
...
export function usePlayGuard() {
  ...
  useEffect(() => {
    ...
    const inMain = (e: Event) => { if (inside(e.target, 'main')) e.preventDefault(); };
    ...
    const zoomed = () => {
      if (!vv || vv.scale <= ZOOMED || !isPlaying(useGame.getState())) return;
