...
export default function FieldGuide({ onClose }: { onClose: () => void }) {
  ...
  return <Modal title="Field guide" onClose={onClose}>
    ...
    <p>Trackpad only: click the open scene to lift and cruise. Move the pointer to steer; hold it near an edge to keep turning. Scroll up with two fingers to accelerate, or down to slow. Click again to brake and hover. While hovering, click and drag to look around without moving. Aim down at a nearby flat surface, then click Land when its ring appears. Moving onto a button or outside the scene stops cruise. Pause and resume with the on-screen buttons.</p>
