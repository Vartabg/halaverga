...
export default function FieldGuide({ onClose }: { onClose: () => void }) {
  ...
  return <Modal title="Field guide" onClose={onClose}>
    ...
    <p>Recommended on a trackpad: click the open scene (with the blaster off it also lifts into hover). Slide one finger to look, hold W/S to fly forward/backward and A/D to move sideways. Release the movement keys to hover. R/F rise and descend, Shift toggles Surge, and Space lifts or lands. A click stops and frees the pointer (with the blaster on it fires; see below). Escape pauses. You can also drag to look or use the arrow keys without capturing the pointer.</p>
    ...
    <h3>Suit blaster</h3>
    <p>Rogue security drones still patrol the district. Aiming slows you to a precise hover-strafe; firing alone keeps you flying at a capped speed. Mouse: once the mouse is captured, hold the left button to fire and the right button to aim. One finger + keyboard: slide to look · click to fire (hold for auto) · hold Q to aim · WASD fly · Space lift/land · Esc pause; the capturing click never fires or lifts. Other trackpad modes: hold C to fire and Q to aim; clicks and scrolling keep their flight meanings. Touch: the suit fires by itself, without slowing you, when the crosshair rests on a drone for a moment. Turn off Auto-fire in Flight settings to get a Fire button: hold it and drag the same finger to aim. Tap controls: Fire and Aim toggles; Fire stops after 3 seconds, and Stop halts movement only. The weapon overheats after long bursts and cools by itself. Aim and tap controls are under More controls in Flight settings. Turn the blaster off in Flight settings.</p>
