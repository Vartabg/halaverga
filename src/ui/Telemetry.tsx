...
export default function Telemetry() {
  const [sample, setSample] = useState({ altitude: 20, position: runtime.position.toArray(), speed: 0, renderedPosition: presentation.position.toArray(), lean: 0, cameraDistance: 0, yaw: runtime.yaw, pitch: runtime.pitch });
  const flying = useGame(s => s.flying), landing = useGame(s => s.landing);
  useEffect(() => { const interval = setInterval(() => setSample({ altitude: Math.round(runtime.altitude), position: runtime.position.toArray(), speed: runtime.speed, renderedPosition: presentation.position.toArray(), lean: presentation.lean, cameraDistance: runtime.cameraDistance, yaw: runtime.yaw, pitch: runtime.pitch }), 350); return () => clearInterval(interval); }, []);
