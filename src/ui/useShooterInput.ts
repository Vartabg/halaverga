...
export function useShooterInput(options?: { unlock?: () => void }) {
  ...
  useEffect(() => {
    ...
    // Desktop only: iPhone Safari fires gesturestart for any second finger, which would drop a two-thumb Fire hold.
    const zoom = () => { if (on() && s.input.touchId === null) resetShooterInput(s); };
    ...
    if (fine) { window.addEventListener('wheel', wheel, passive); window.addEventListener('gesturestart', zoom); }
    return () => {
      ...
      if (fine) { window.removeEventListener('wheel', wheel, passive); window.removeEventListener('gesturestart', zoom); }
      resetShooterInput(s);
    };
  }, []);
}
