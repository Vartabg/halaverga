...
export default function ShooterSettings() {
  ...
  return <fieldset><legend>Suit blaster</legend>
    <label className={styles.check}><input type="checkbox" checked={shooter} onChange={e => {
      // Off returns assist friction, blends and springs to exact rest so look input is never left slowed.
      if (e.target.checked) clearShooterFault(); else resetShooterFeel(runtime.shooter);
      save({ shooter: e.target.checked });
