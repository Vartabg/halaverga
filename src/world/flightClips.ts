...
// The chest arches, the shoulders draw back and the arms sweep back in a narrow delta, the hands a hand's width out from the hips;
// the legs trail back together from the hips with the knees soft, fluttering at 1 Hz in antiphase (the left side runs .5 s behind),
// each foot flicking .15-.3 s after its thigh. One clip for both styles.
const cruise: AuthoredClip = { name: 'cruise', duration: 2, loop: true, source: 'authored-table', keys: mirrorKeys({
  ...
  upperarm_r: [[0, -.43, 0, .09], [.5, -.45, 0, .07], [1.5, -.41, 0, .11]], forearm_r: [[0, .1, 0, 0]],
  hand_r: [[0, -.3, 0, .1], [.7, -.38, 0, .1], [1.7, -.25, 0, .1]],
  thigh_r: [[0, -.3, 0, -.035], [.5, -.16, 0, -.035], [1, -.3, 0, -.035], [1.5, -.16, 0, -.035]],
  shin_r: [[.15, -.3, 0, 0], [.65, -.6, 0, 0], [1.15, -.3, 0, 0], [1.65, -.6, 0, 0]],
...
const powerHero: AuthoredClip = { ...power, name: 'powerHero', keys: { ...powerKeys,
  spine: [[0, .05, 0, 0]], chest: [[0, .1, .02, 0], [.5, .11, .02, 0]], clavicle_l: [[0, 0, .08, .06]],
  upperarm_l: [[0, -.3, 0, .12], [.5, -.28, 0, .11]], forearm_l: [[0, .04, 0, 0], [.4, .07, 0, 0]], hand_l: [[0, -.35, 0, 0], [.5, -.42, 0, 0]],
...
// An asymmetric flare: the right arm reaches forward at chest height with the elbow well bent, the left lower and a little wider
// with the elbow bent too; the right knee drives up higher and a little out, the left stays under the hip. A hunch, not an arch, so the back stays
...
const brakeKeys: Keys = {
  ...
  upperarm_r: [[0, 1.3, -.1, -.05], [.6, 1.34, -.1, -.03], [1.4, 1.26, -.1, -.07]], forearm_r: [[0, .9, 0, 0], [.8, .98, 0, 0]],
  upperarm_l: [[0, .7, .1, -.36], [.4, .67, .1, -.34], [1.6, .73, .1, -.38]], forearm_l: [[0, .85, 0, 0], [1.8, .92, 0, 0]],
...
const brakeHero: AuthoredClip = { ...brake, name: 'brakeHero', keys: { ...brakeKeys,
  upperarm_r: [[0, 1.4, -.15, -.05], [.6, 1.43, -.15, -.03], [1.4, 1.37, -.15, -.07]], upperarm_l: [[0, .65, .1, -.38], [.4, .62, .1, -.36], [1.6, .68, .1, -.4]],
  forearm_r: [[0, 1, 0, 0], [.8, 1.06, 0, 0]], forearm_l: [[0, .88, 0, 0], [1.8, .95, 0, 0]],
...
// shoulders drawn back and the arms straight behind with the hands brought in together behind the hips, a small flutter.
const climb: AuthoredClip = { name: 'climb', duration: 2, loop: true, source: 'authored-table', keys: mirrorKeys({
  clavicle_r: [[0, 0, -.2, -.05]], upperarm_r: [[0, -.42, 0, -.17], [1, -.44, 0, -.19]], forearm_r: [[0, .05, 0, 0]], hand_r: [[0, -.2, 0, -.3]],
