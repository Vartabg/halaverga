...
// Left turn is positive weight: the torso side-bends into the turn with the head a little counter-rolled; both legs swing together
// toward the outside of the turn, straight, so the toes keep their spacing (flightPose fades the legs out at power speed). The arms
// stay close (the whole-body roll carries the turn): the outside arm draws in against the side-bend so it stays along the body, the
// inside arm bends at the elbow and stays back, clear of the thigh.
const bankLeft = still('bankLeft', {
  ...
  thigh_l: [[0, 0, 0, .18]], thigh_r: [[0, 0, 0, .18]],
});
