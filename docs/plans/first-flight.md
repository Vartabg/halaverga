...
- **Phone Standard**: look acceleration is on by default, so a fast swipe turns further (a 350 px swipe in 200 ms turns about 270°, was 104°) while slow aiming is unchanged. A thumb that arrives fast at the outer edge or at the stick-zone boundary and rests there keeps turning (two bands, so left and right both work). One-thumb: a thumb near the edge keeps turning.
- **Desktop Standard**: the edge hold keeps turning for as long as it is held. Hovering the header keeps steering, a side exit from the window keeps turning for up to 6 s, and a top or bottom exit flies straight after 1 s.
...
- **Brush**: a big loop is a whirl that spins you round (360 in about 1.3 s), and same-direction swipes stack into one turn.
