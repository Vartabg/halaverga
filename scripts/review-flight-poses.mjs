...
const cases=[['HOVER',0,0,0,0,0],['POWER FLIGHT',34,-1.35,0,0,0],['SWEEPING BANK',24,-1.15,-.3,0,0],['RISING ARC',24,-1.15,0,.8,0],['DIVE',24,-1.15,0,-.8,0],['AIR BRAKE',12,-.35,0,0,.85]];
for(const [name,speed,lean,bank,climb,brake]of cases){
 ...
 const camera=new T.PerspectiveCamera(35,440/460,.1,20);camera.position.set(3,1.3,-4.2);camera.lookAt(0,0,-.1);renderer.render(scene,camera);
...
try {
 ...
 await page.route('**/pose-study/**',async route=>{
  ...
  if(!['world/suitRig','world/suitGeometry','world/suitPose','game/presentation'].includes(relative))return route.abort();
