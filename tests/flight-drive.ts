...
/** The physics state a `step` hook may change after each 60 Hz step (a wall deflection, a touchdown). */
export type Body = { t: number; yaw: number; pitch: number; v: Vec; flying: boolean; landing: boolean };
export type Options = {
  hz?: number; pitch?: number; hero?: number; seconds?: number; reduced?: (t: number) => boolean; rig?: Rig; step?: (b: Body) => void;
};
...
export function drive(device: Device, { hz = 60, pitch = 0, hero = 1, seconds = 5, reduced = () => false, rig, step }: Options, each: (f: Frame) => void) {
  const s = device.speed, p = flightPose({ speed: s, viewPitch: pitch, pitch }), life = cruising(), mix = createFlightMix(), turn = createSuitRoll();
  ...
  for (let f = 0; f < Math.round(seconds * hz); f++) {
    ...
    for (due += dt; due >= 1 / 60 - 1e-9; due -= 1 / 60) {
      ...
      body.t += 1 / 60; step?.(body);
    }
    const v = body.v, input = { paused: false, reduced: calm, flying: body.flying, landing: body.landing, velocity: v };
