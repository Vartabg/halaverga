...
describe('controlsHint', () => {
  ...
  it('gives each input profile its own line', () => {
    ...
    expect(env(false, 'trackpad', 'simple')).toBe('CLICK FIRES · HOLD Q TO AIM · ESC FREES POINTER');
...
describe('HUD sources', () => {
  ...
  it('stay landing-safe, allocation-light and inside the 200 line budget', () => {
    for (const f of ['hudTimeline.ts', 'ShooterHud.tsx', 'ShooterSettings.tsx', 'ShooterHud.module.css']) {
      const src = read(f);
