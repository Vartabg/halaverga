...
export type Result = { name: string; tips: number[]; hands: number; feet: number; px: number };
/**
...
export function silhouette(s: State, reduced = false): Result {
  const sum = [0, 0, 0, 0]; let n = 0, best = -1, peak: number[] = [], px = 0;
  play(s.warm + s.span, 60, s.drive, shot => {
    ...
    if (s.pick === 'brake') {
      ...
      if (w > best) { best = w; peak = displacement(shot); px = pixels(1, shot); }
    } else if (s.pick === 'flare') {
      if (shot.life.flying && shot.mix.flare > .5) { peak = displacement(shot); px = pixels(1, shot); }
    } else { displacement(shot).forEach((d, k) => { sum[k] += d; }); n++; px = pixels(1, shot); }
