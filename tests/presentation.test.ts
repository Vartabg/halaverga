...
const fresh = (): Pose => ({ viewYaw: 0, viewPitch: 0, yaw: 0, pitch: 0, lean: 0, bank: 0, speed: 0, flight: 0, power: 0, brake: 0 });
...
describe('composed flight presentation', () => {
  ...
  it('converges to the same streamlined pose at 30, 60 and 120 Hz', () => {
    ...
    for (const p of results) {
      expect(p.lean).toBeCloseTo(-1.35, 2); expect(p.speed).toBeCloseTo(34, 2); expect(p.power).toBeCloseTo(1, 2);
  ...
  it('takes the short turn across the angle wrap and returns smoothly to hover', () => {
    ...
    expect(Math.abs(p.lean)).toBeLessThan(.001); expect(Math.abs(p.bank)).toBeLessThan(.001); expect(p.power).toBeLessThan(.001);
  });
