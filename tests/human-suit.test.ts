...
test('weighted poses remain finite and disposing a rig leaves the loaded source reusable', async () => {
  ...
  for (let n = 0; n < 3; n++) {
    ...
    for (const brake of [0, .5, 1]) {
      applySuitPose(rig.joints, { viewYaw: 0, viewPitch: 0, yaw: 0, pitch: .8, lean: -1.35, bank: .3, speed: 34, flight: 1, power: 1, brake }, { hero: 1, epoch: 0 }, false);
      rig.root.updateMatrixWorld(true);
...
test('hovering hands hang in front of the elbows and a braking heel tucks behind its knee', async () => {
  ...
  try {
    const hover = { viewYaw: 0, viewPitch: 0, yaw: 0, pitch: 0, lean: 0, bank: 0, speed: 0, flight: 1, power: 0, brake: 0 };
    ...
    const brake = { ...hover, speed: 10, power: .28, lean: -.258, brake: 1 };
