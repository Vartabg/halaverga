...
describe('adaptive thumbs with an external Fire owner', () => {
  ...
  it('blocks a second surface contact while external (Fire counts as a thumb)', () => {
    const t = new AdaptiveThumbs(); t.setExternal(true, 0);
    t.start(1, 90, 650); t.start(2, 290, 650);
    expect(t.mode).toBe('blocked'); expect(t.active).toBe(false);
    const both = new AdaptiveThumbs(); both.start(1, 90, 650); both.start(2, 290, 650);
    both.setExternal(true, 0); expect(both.mode).toBe('blocked');
  });
  it('releases to an inactive single thumb that does not launch until it slides', () => {
    ...
    t.move(1, 121, 590, W, H); expect(t.active).toBe(false); expect(t.output.forward).toBe(0);
    t.move(1, 140, 590, W, H); expect(t.active).toBe(true); expect(t.output.forward).toBeGreaterThan(0);
